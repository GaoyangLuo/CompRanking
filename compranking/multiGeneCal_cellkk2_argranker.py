...
def RB_gene_sum(DB_deepARG_length,DB_SARG_length, DB_MobileOG_length, 
    ...
    for orf in Record_db_orf:
        ...
        if Record_db_orf[orf]:
            ...
            if find_db=="DeepARG":
                ...
                abundance_arg_RPKM += mapped_reads / DB_deepARG_length_res[orf] * gene_length / copy_16S #argranker AGS / kk2_standard database
                TAXO_ARG.setdefault(str(orf), float((mapped_reads / DB_deepARG_length_res[orf]) / copy_16S))#kk2_standard database
                RPKM_ARG.setdefault(str(orf), float(mapped_reads / DB_deepARG_length_res[orf] * gene_length / copy_16S))#argranker
            elif find_db=="RGI":
                abundance_arg_16S += (mapped_reads / DB_CARD_length_res[orf]) / copy_16S #kk2_standard database
                abundance_arg_RPKM  += mapped_reads / DB_CARD_length_res[orf] * gene_length / copy_16S #argranker
                TAXO_ARG.setdefault(str(orf), float((mapped_reads / DB_CARD_length_res[orf])/ copy_16S)) #kk2_standard database
                RPKM_ARG.setdefault(str(orf), float(mapped_reads / DB_CARD_length_res[orf] * gene_length / copy_16S))#argranker
            elif find_db=="SARG":
                abundance_arg_16S += (mapped_reads / DB_SARG_length_res[orf]) / copy_16S #kk2_standard database
                abundance_arg_RPKM  += mapped_reads / DB_SARG_length_res[orf] * gene_length / copy_16S#argranker
                TAXO_ARG.setdefault(str(orf), float((mapped_reads / DB_SARG_length_res[orf]) / copy_16S)) #kk2_standard database
                RPKM_ARG.setdefault(str(orf), float(mapped_reads / DB_SARG_length_res[orf] * gene_length / copy_16S))#argranker
            else:
                continue
    ...
    for orf_MGE in DB_MobileOG_length_res:
        ...
        abundance_MGE_RPKM += mapped_reads / DB_MobileOG_length_res[orf_MGE] * gene_length / copy_16S #argranker
        TAXO_MGE.setdefault(str(orf_MGE),float((mapped_reads / DB_MobileOG_length_res[orf_MGE]) / copy_16S)) #kk2_standard database
        RPKM_MGE.setdefault(str(orf_MGE),float(mapped_reads / DB_MobileOG_length_res[orf_MGE] * gene_length / copy_16S))#argranker
