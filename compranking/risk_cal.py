#!/usr/bin/env python
# title             :risk_cal.py -> risk_cal.ipynb
...
with open(write_file, "w") as f1:
    f1.write("sample_name/index\tnContigs\tnARGs_contigs\tnMGEs_contig\tnMGEs_plasmid_contig\tnMGEs_phage_contigs\tnPAT_pathogenic_contigs\tnPAT_all_contigs\tnARGs_MGEs_contig\tnARGs_MGEs_plasmid_contigs\tnARGs_MGEs_phage_contigs\tnARGs_MGEs_PAT_pathogenic_contigs\tnARGs_MGEs_allPAT_contigs\tfARG\tfMGE\tfMGE_plasmid\tfMGE_phage\tfPAT_pathogenic\tfARG_MGE\tfARG_MGE_plasmid\tfARG_MGE_phage\tfARG_MGE_PAT_pathogenic\tfARG_MGE_allPAT\tscore_all\tscore_pathogenic\tscore_phage\tscore_plasmid")

for i in sample_list: #获取sample_name
    sample_name=i
    print(sample_name)
    # 获取结果表
    file_path=os.path.join(input_dir+ sample_name +"_MGE_prediction.txt")
    ...
    #nMGEs_counts
    MGEs_filter=df[df.peram_MGE_prediction!="unclassified" ]
    MGEs_filter=MGEs_filter[MGEs_filter.peram_MGE_prediction!="chromosome"]
    MGEs_filter=MGEs_filter[MGEs_filter.peram_MGE_prediction!="ambiguous (phage/chromosome)"]
    nMGEs_contigs=len(MGEs_filter.Contig.unique()) # numbers of contigs containing predicted MGEs
    #nMGEs_plasmids_counts
    MGEs_plasmid_filter=df[df.peram_MGE_prediction=="plasmid" ]
    nMGEs_plasmid_contigs=len(MGEs_plasmid_filter.Contig.unique())
    #nMGEs_phage_counts
    MGEs_phage_filter=df[df.peram_MGE_prediction=="phage" ]
    nMGEs_phage_contigs=len(MGEs_phage_filter.Contig.unique())
    #nPAT(pathogenic)_counts
    PAT_filter=df[df.Virulence_prediction=="pathogenic"]
    nPAT_pathogenic_contigs=len(PAT_filter.Contig.unique())
    #nPAT_all_counts
    allPAT_filter=df[df.Virulence_prediction!="unclassified"]
    nPAT_all_contigs=len(allPAT_filter.Contig.unique())
    #nARGs_MGEs_counts
    #RankII_Risk
    nARGs_MGEs_filter=arg_filter[arg_filter.peram_MGE_prediction!="unclassified"]
    ...
    nARGs_MGEs_allPAT_filter=nARGs_MGEs_filter[nARGs_MGEs_filter.Virulence_prediction!="unclassified"]
    nARGs_MGEs_allPAT_contigs=len(nARGs_MGEs_allPAT_filter.Contig.unique())
    #nARGs_MGEs_PAT(pathogenic)
    #RankIII_Risk
    nARGs_MGEs_PAT_filter=nARGs_MGEs_filter[nARGs_MGEs_filter.Virulence_prediction=="pathogenic"]
    nARGs_MGEs_PAT_pathogenic_contigs=len(nARGs_MGEs_PAT_filter.Contig.unique())
    #risk calculation
    # nContigs
    # nARGs_contigs
    # nMGEs_contigs
    ...
    fARG_MGE_PAT_pathogenic= float(nARGs_MGEs_PAT_pathogenic_contigs)/nContigs
    fARG_MGE_allPAT=float(nARGs_MGEs_allPAT_contigs)/nContigs
    ...
    distance_pathogenic = math.sqrt((0.01 - fARG)**2 + (0.01 - fARG_MGE)**2 + (0.01 - fARG_MGE_PAT_pathogenic)**2)
    distance_phage=math.sqrt((0.01 - fARG)**2 + (0.01 - fARG_MGE_phage)**2 + (0.01 - fARG_MGE_PAT_pathogenic)**2)
    distance_plasmid= math.sqrt((0.01 - fARG)**2 + (0.01 - fARG_MGE_plasmid)**2 + (0.01 - fARG_MGE_PAT_pathogenic)**2)
