    ...
    for orf in Record_db_orf:
        ...
        if Record_db_orf[orf]:
            ...
            if find_db=="DeepARG":
                ...
                abundance_arg_16S += 0
                abundance_arg_RPKM += 0
                TAXO_ARG.setdefault(str(orf), float(0))
                RPKM_ARG.setdefault(str(orf), float(0)) #rpkg
            elif find_db=="RGI":
...
def Calculation(file_name_base):
    ...
    try:
        ...
        with open(os.path.join(input_dir,project_prefix,"CompRanking_result/Gene_Abundance_Sum_rpkg_SARG.txt"), "a") as f:
            f.write("\n" + i + "\t" + output_abundance)
    except:
...
if __name__ == "__main__":
    ...
    df_main.to_csv(os.path.join(
                    input_dir,project_prefix,
                        "CompRanking_result",
                            project_prefix+"_Abundance_ARGs_subtypes_rpkg_SARG.txt"),sep="\t",index=None)
    #concat MGE result
    #concat 16S
    name_list_16S=[]
    for i in file_name_base:
    ...
    df_main.to_csv(os.path.join(
                    input_dir,project_prefix,
                        "CompRanking_result",
                            project_prefix+"_Abundance_MGEs_subtypes_rpkg_SARG.txt"),sep="\t",index=None)
