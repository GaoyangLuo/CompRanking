...
# description       :This script is for benchmarking using genome equivalents as normalization bases to gener
#                    generate risk score
# author            :Gaoyang Luo
# date              :202201119
# version           :1.0
...
def info_sum(sample_list, genome_equivalents):
# for i in sample_list: #获取sample_name
    ...
    nContigs=float(genome_equivalents)
    print("genome_equivalents is {}".format(nContigs))
    ...
    for index, j in enumerate(input_AGS): 
        for lines in open(j,'r'):
                            if lines.startswith('genome_equivalents'):
                                ...
                                print("The Genome Equivalent of file {} is {}".format(prefix_list[index],genome_equivalents))
...
def multi_info_sum():
    ...
    for i in range(openthreads):
        worker = multiprocessing.Process(target=info_sum,args=([sample_list[i],genome_equivalents_dic[sample_list[i]]]))
        worker.start()
